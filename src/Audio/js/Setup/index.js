...
async function setupAudio(tuneDetectionCallback) {
  ...
  try {
    ...
    const response = await window.fetch("wasm-audio/rs_bg.wasm");
    const wasmBytes = await response.arrayBuffer();

    // Add our audio processor worklet to the context.
    const processorUrl = "Audio/Worker/index.js";
    try {
    ...
    node = new Dispatcher(context, "AudioWorker");
    ...
    node.init(wasmBytes, tuneDetectionCallback, numAudioSamplesPerAnalysis);
